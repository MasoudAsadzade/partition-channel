...
namespace PTPLib {
    ...
    class safe_ptr {
        ...
        class auto_lock_t {
            ...
            req_lock lock;
        public:
            auto_lock_t(auto_lock_t && alt) : ptr(std::move(alt.ptr)), lock(std::move(alt.lock)) {}
        ...
        class auto_lock_obj_t {
            ...
            req_lock lock;
        public:
            auto_lock_obj_t(auto_lock_obj_t && alot) : ptr(std::move(alot.ptr)), lock(std::move(alot.lock)) {}
        ...
        template<size_t, typename, size_t, size_t> friend
        class lock_timed_any;

        template<class mutex_type> friend
